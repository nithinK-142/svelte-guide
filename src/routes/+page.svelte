<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			error = 'Failed to fetch guide info, try again after sometime!';
			loading = false;
		}
	});
</script>

<h1 class="pt-10 text-3xl text-center text-white">SvelteKit Guide</h1>

{#if loading}
	<p class="mt-10 text-xl tracking-wider text-center text-white animate-pulse">Loading...</p>
{:else if error}
	<p class="mt-10 text-xl text-center text-red-500">{error}</p>
{:else}
	<div class="mt-10 min-w-[18rem] sm:min-w-[28rem] sm:floating-scrollbar">
		{#each branches as { branchName, vercelLink }}
			<div
			...
			>
				<div class="inline-flex items-center h-4">
					...
					<a
						href={`https://github.com/nithinK-142/svelte-guide/tree/${branchName}`}
						target="_blank"
				...
				{#if vercelLink}
					<a
						href={`https://svelte-guide-git-${vercelLink}-nithink142s-projects.vercel.app`}
						target="_blank"
